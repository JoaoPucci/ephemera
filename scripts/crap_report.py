...
# CRAP score bands. The 30 boundary is the conventional "hard to
# safely change" threshold from the original Crap4j paper (Schauder /
# Savoia, 2007); the rest of the bands extrapolate around it. The
# action column is the load-bearing column -- the band labels are
# just shorthand. Tweak as the codebase soaks at known scores and we
# learn what "normal" looks like for ephemera.
SCORE_BANDS = [
    (15, "Healthy", "Simple or well-covered. No action."),
    (
        30,
        "Watch",
        "Borderline. Would be more comfortable with more coverage or a smaller method.",
    ),
    (
        80,
        "Refactor",
        "Above the conventional 30-point ceiling. Add tests or split.",
    ),
    (
        float("inf"),
        "Critical",
        "Branchy AND poorly tested. Pick whichever lever is cheaper (tests vs split) and apply it.",
    ),
]


def _band_label(crap_score: float) -> str:
    for upper, label, _ in SCORE_BANDS:
        if crap_score <= upper:
            return label
    return SCORE_BANDS[-1][1]  # unreachable; the last band is unbounded


def _render_legend() -> list[str]:
    """The classification table that turns raw CRAP numbers into an
    action. Rendered after the ranked rows so a reader can scan the
    rows top-down then cross-reference the score band here."""
    lines = [
        "### How to read CRAP",
        "",
    ...
    prev = 0
    for upper, label, action in SCORE_BANDS:
        if upper == float("inf"):
            range_label = f"&gt; {prev}"
        elif prev == 0:
            range_label = f"≤ {int(upper)}"
        else:
            range_label = f"{prev + 1}–{int(upper)}"
        lines.append(f"| {range_label} | {label} | {action} |")
        if upper != float("inf"):
            prev = int(upper)
    return lines
...
def render_markdown(rows: list[dict], top: int) -> str:
    ...
    lines = [
        ...
        "",
        "| CRAP | Band | Complexity | Coverage | Method |",
        "| ---: | --- | ---: | ---: | --- |",
    ]
    for row in rows[:top]:
        ...
        band = _band_label(row["crap"])
        lines.append(
            f"| {crap} | {band} | {row['complexity']} | {cov_pct} "
            f"| {method}:{row['lineno']} |"
        )
    lines.append("")
    lines.append(
